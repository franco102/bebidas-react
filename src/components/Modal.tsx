...
export default function Modal() {
  ...
  const renderIngrdients=()=>{
    const ingredients:JSX.Element[]=[];
    for (let index = 1; index < 6; index++) {
      const ingredient=selectedRecipe[`strIngredient${index}` as keyof Recipe];
      const strMeasure=selectedRecipe[`strMeasure${index}` as keyof Recipe];
      
      if(ingredient && strMeasure){
        ingredients.push(
          <li key={index} className='text-lg font-normal'>{strMeasure} - {ingredient}</li>
        );
      }
    }
    return ingredients
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6" >
                  ...
                  <img src={selectedRecipe.strDrinkThumb} alt="receta" className='mx-auto w-96' />
                  ...
                  <div className='mt-5 flex justify-between gap-4'>
                    <button 
                      ...
                      className='w-full rounded bg-orange-600 p-3 font-bold uppercase text-white shadow hover:bg-orange-700'>
                        {favoriteExists(selectedRecipe.idDrink)?'Eliminar a Favorito':'Agregar a Favorito'}
                      </button>
                  </div>
                </Dialog.Panel>
              </Transition.Child>
